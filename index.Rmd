...
for (cod in cod_list) {
  ...
  p <- ggplot(data0, aes(x = date, y = ndvi)) +
    ...
    labs(title = paste("NDVI Trend for COD:", cod),
         x = "Time",
         y = "NDVI") +
    theme_minimal()
  
  # Save results in the list
  results_list[[cod]] <- list(intercept_ts = intercept_ts, slope_ts = slope_ts, mk = mk_test, plot = p)
...
for (cod_value in cod_names_0) {
  ...
  ts_data <- ts(data0$ndvi, start = c(2019, 1), end = c(2023, 12), frequency = 12) ##### !!!!
...
### **Load the crowns shapefile**

```{r Load the crowns shapefile_1, eval=FALSE}
crowns0 <- st_read("./INPUT/VECTOR/crowns.gpkg")

crowns <- st_transform(crowns0, crs = st_crs("+proj=longlat +datum=WGS84"))

st_write(crowns, "./INPUT/VECTOR/crowns_wgs84.gpkg", append = FALSE )
```

```{r Load the crowns shapefile_2, eval=FALSE}
# Clean the R environment
rm(list=ls())
```
...
# Load the shp file of the plots in the Paulilatino area
crowns_wgs84 <- st_read("./INPUT/VECTOR/crowns_wgs84.gpkg")
```
...
```{r}
# Visualize the correlation between the slope and the p-value
cor  <- cor(pixel_trend$slope, pixel_trend$mk_p_value)
...
plot(pixel_trend$slope, pixel_trend$mk_p_value, main = "Correlation between slope and p-value", xlab = "Slope", ylab = "P-value")
```


```{r}
# You have all the code and the process...what I can do after that? I would like to improve the elaboration of the data and the visualization of the results. Write a code of example to show me how to do it. Write it no row by row.
```
